...
**Problem:** A new Midnight user today faces seed management, three wallet types, name registration, credential issuance, and recovery setup --- all before they can use a single dApp. No unified onboarding path exists.

**Solution:** A single QR code scan that takes a user from zero state to a named, credentialed, recoverable account in ~60 seconds. This repo contains the design spec, investigation notes, and open research threads that drive requirements for Nightstream and the Starstream mock-ledger.
...
| Stream | Key Contribution |
|---|---|
| [CAKE Framework](https://frontier.tech/the-cake-framework) | Intent-based cross-chain routing, policy-based TEE agents |
| [OpenWallet Standard](https://openwallet.sh/) | Verifiable credentials, holder-controlled selective disclosure |
| [DeRec Alliance](https://derecalliance.org/) | Shamir SS with Merkle commitments, (3,5) threshold, ML-KEM post-quantum |
| [NEAR Protocol](https://docs.near.org/protocol/accounts-contracts/account-model) | Multi-key accounts, function-call keys, device-centric key management |
| [zkMe](https://docs.zk.me/hub) | zkKYC via Groth16, FHE biometrics, Soulbound Tokens |
| [ENS](https://ens.domains/) | Namehash, registry/resolver, CCIP-Read wildcards, Name Wrapper fuses |
| TEE Research | iOS Secure Enclave, Android StrongBox, ARM CCA, AES-256-GCM wrapping |
| Midnight Platform | BLS12-381/Jubjub, Compact circuits, attestation trees, DUST fees |
